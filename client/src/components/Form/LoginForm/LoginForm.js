...
function LoginForm() {
   ...
   const {
      ...
      resolver: yupResolver(
         createYupSchema(
            config.itemList.authFormData[authMode].inputs.map(item => item.name)
         )
      ),
   });
         ...
         <div className="">
            ...
            <div>
               <form onSubmit={handleSubmit(onSubmit)}>
                  <div>
                     {config.itemList.authFormData[authMode].inputs.map((input, index) => (
                        <FormInput
                           {...input}
                           register={register}
                           error={errors[config.itemList.authFormData[authMode].inputs[index].name]}
                           key={index}
                        />
                     ))}
                  </div>
                  <div className="flex justify-between py-6">
                     {authMode === 'login' ? (
                        ...
                        <Link to="/login" className="text-sm text-sky-600">
